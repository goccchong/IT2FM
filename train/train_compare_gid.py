...
from compare_methods.WFENet import WFENet
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        #frame_work = DeepLab(num_classes=6)
        #frame_work = PSPNet(num_classes=6,downsample_factor=8)
        #frame_work = UNetFormer(num_classes=6)
        #frame_work = FuzzyNet(n_class=6)
        #frame_work = PoolFormer(num_classes=6)
        #frame_work = BANet(num_classes=6, weight_path=None)
        #frame_work = MSResNet(num_classes=6)
        #frame_work = SFFNet(num_classes=6)
        #frame_work = A2FPN(class_num=6)
        #frame_work = WFENet(n_channels=3, n_classes=6)
        frame_work = MIFNet(num_classes=6)
    ...
    for epoch in range(epoches):
        ...
        with open('./A_modelpath/gid/MIFNet/MIFNet_training_loss.txt', 'a') as file:
        ...
        if epoch % 50 == 0:
            torch.save(model.state_dict(), "./A_modelpath/gid/MIFNet/MIFNet-%d.pth" % (model_id + epoch + 1))
        end = time.time()
        time_cha = end - start
...
if __name__ == '__main__':
    ...
    data_dir = '/home/ps/data/code/wgy/1Awork_02/Datasets/A_GID/'
    batch_size = 8
    model_dir = './A_modelpath/gid/MIFNet/'
